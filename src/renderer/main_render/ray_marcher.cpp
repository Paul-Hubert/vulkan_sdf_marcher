...
RayMarcher::RayMarcher(Context& ctx) : ctx(ctx) {
    
    initPipeline();
    
}

void RayMarcher::render(vk::CommandBuffer commandBuffer) {
    
    commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0, {ctx.main_render.ubo.descSets[ctx.frame_index]}, {});
...
void RayMarcher::initPipeline() {
    
    // PIPELINE INFO
    
    auto vertShaderCode = Util::readFile("./resources/shaders/raymarcher.vert.glsl.spv");
    auto fragShaderCode = Util::readFile("./resources/shaders/raymarcher.frag.glsl.spv");
    ...
    auto vertexInputState = vk::PipelineVertexInputStateCreateInfo({}, 0, nullptr, 0, nullptr);
    ...
    auto layouts = std::vector<vk::DescriptorSetLayout> {ctx.main_render.ubo.descLayout};
    ...
    pipeline = ctx.device->createGraphicsPipelines(nullptr, {pipelineInfo}).value[0];
